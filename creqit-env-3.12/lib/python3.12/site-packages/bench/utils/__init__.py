...
sudoers_file = "/etc/sudoers.d/creqit"
UNSET_ARG = object()
...
def is_creqit_app(directory: str) -> bool:
	is_creqit_app = True

	for folder in paths_in_app:
		if not is_creqit_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_creqit_app = is_creqit_app and path

	return bool(is_creqit_app)
...
def is_valid_creqit_branch(creqit_path: str, creqit_branch: str):
	...
	:param creqit_path: git url
	:type creqit_path: str
	:param creqit_branch: branch to check
	:type creqit_branch: str
	...
	if creqit_branch:
		try:
			res = g.ls_remote("--heads", "--tags", creqit_path, creqit_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {creqit_branch} for the remote {creqit_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid creqit path: {creqit_path}") from e
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/creqit-bench/json")
	except Exception:
...
def run_creqit_cmd(*args, **kwargs):
	...
	p = subprocess.Popen(
		(f, "-m", "creqit.utils.bench_helper", "creqit") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_env_creqit_commands(bench_path=".") -> List:
	"""Caches all available commands (even custom apps) via Creqit
	...
	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m creqit.utils.bench_helper get-creqit-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo, using_cached: bool = False):
	...
	raise InvalidRemoteException(
		f"{org_repo} not found under creqit or erpnext GitHub accounts"
	)
...
class _dict(dict):
	...
	# bench port of creqit._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix creqit#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `bench --profile --site creqitframework.com migrate --no-backup`
	sys.argv: ["/home/creqit/.local/bin/bench", "--profile", "--site", "creqitframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to creqit's bench_helper
	from bench.bench import Bench

	creqit_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in creqit_context.flags:
			continue

		elif arg in creqit_context.params:
			skip_next = True
			continue
...
def get_app_cache_extract_filter(
	...
	def filter_function(member: TarInfo, dest_path: str) -> Optional[TarInfo]:
		...
		try:
			...
			# Links created by `creqit` after extraction
			return None
