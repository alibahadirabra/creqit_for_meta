...
def init(
	...
	* setup backups crontab
	...
	# remote apps
		...
		creqit_path = creqit_path or "https://github.com/creqit/creqit.git"
		is_valid_creqit_branch(creqit_path=creqit_path, creqit_branch=creqit_branch)
		get_app(
			creqit_path,
			branch=creqit_branch,
			bench_path=path,
			skip_assets=True,
			verbose=verbose,
			resolve_deps=False,
...
def setup_sudoers(user):
	...
	template = bench.config.env().get_template("creqit_sudoers")
	creqit_sudoers = template.render(
		**{
			"user": user,
...
def migrate_site(site, bench_path="."):
	run_creqit_cmd("--site", site, "migrate", bench_path=bench_path)


def backup_site(site, bench_path="."):
	run_creqit_cmd("--site", site, "backup", bench_path=bench_path)
...
def fix_prod_setup_perms(bench_path=".", creqit_user=None):
	from glob import glob
	from bench.bench import Bench

	creqit_user = creqit_user or Bench(bench_path).conf.get("creqit_user")

	if not creqit_user:
		print("creqit user not set")
		sys.exit(1)

	globs = ["logs/*", "config/*"]
	for glob_name in globs:
		for path in glob(glob_name):
			uid = pwd.getpwnam(creqit_user).pw_uid
			gid = grp.getgrnam(creqit_user).gr_gid
...
def setup_fonts():
	...
	exec_cmd("git clone https://github.com/creqit/fonts.git", cwd="/tmp")
