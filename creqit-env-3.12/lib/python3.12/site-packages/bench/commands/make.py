...
@click.option(
	...
	"creqit_branch",
	default=None,
	help="Clone a particular branch of creqit",
)
@click.option(
...
@click.option("--creqit-path", default=None, help="path to creqit repo")
...
@click.command("new-app", help="Create a new Creqit application under apps folder")
@click.option(
	"--no-git",
	is_flag=True,
	flag_value="--no-git",
	help="Do not initialize git repository for the app (available in Creqit v14+)",
)
...
@click.command(
	...
	help="Validates that all requirements specified in creqit-dependencies are met curently.",
)
@click.pass_context
def validate_dependencies(ctx):
	"Validate all specified creqit-dependencies."
	from bench.bench import Bench
