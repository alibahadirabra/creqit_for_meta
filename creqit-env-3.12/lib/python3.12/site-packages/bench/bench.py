...
class Bench(Base, Validator):
	...
	def uninstall(self, app, no_backup=False, force=False):
		if app == "creqit":
			raise ValidationError("You cannot uninstall the app `creqit`")
		from bench.app import App
	...
	def build(self):
		# build assets & stuff
		run_creqit_cmd("build", bench_path=self.name)
...
class BenchApps(MutableSequence):
	...
	def update_apps_states(
		...
		if self.apps and not os.path.exists(self.states_path):
			...
			# Keeping creqit as the first app.
			if "creqit" in self.apps:
				self.apps.remove("creqit")
				self.apps.insert(0, "creqit")
	...
	def initialize_apps(self):
		try:
			self.apps = [
				...
				if is_creqit_app(os.path.join(self.bench.name, "apps", x))
			]
			self.apps.remove("creqit")
			self.apps.insert(0, "creqit")
...
class BenchSetup(Base):
	...
	def env(self, python="python3"):
		...
		- upgrade env pip
		- install creqit python dependencies
		"""
		import bench.cli
		import click
		...
		creqit = os.path.join(self.bench.name, "apps", "creqit")
		...
		if os.path.exists(creqit):
			self.run(
				f"{self.bench.python} -m pip install {quiet_flag} --upgrade -e {creqit}",
				cwd=self.bench.name,
			)
	...
	def backups(self):
		...
		user = self.bench.conf.get("creqit_user")
