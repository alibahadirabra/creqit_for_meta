...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/creqit/healthcare.git
		        2. git@github.com:creqit/healthcare.git
		        3. creqit/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"creqit/erpnext": "~13.17.0"}]
		"""
...
class App(AppMeta):
	...
	def install(
		...
		if not resolved and self.app_name != "creqit" and not ignore_resolution:
			click.secho(
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("bench", {}).get("creqit-dependencies")
		)
		if not deps:
			return
...
def can_creqit_use_cached(app: App) -> bool:
	min_creqit = get_required_creqit_version(app)
	if not min_creqit:
		return False

	try:
		return sv.Version(min_creqit) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_creqit)
	except ValueError:
		click.secho(
			f"Bench app-cache: invalid value found for creqit version '{min_creqit}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	...
	if not dep_path.is_dir():
		click.secho(f"Required creqit-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed creqit-dependency '{dep}' version '{dep_version}' "
...
def get_required_creqit_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/creqit/bench/issues/1524
	req_creqit = (
		pyproject.get("tool", {})
		.get("bench", {})
		.get("creqit-dependencies", {})
		.get("creqit")
	)

	if not req_creqit:
		click.secho(
			"Required creqit version not set in pyproject.toml, "
			"please refer: https://github.com/creqit/bench/issues/1524",
			fg="yellow",
		)
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Creqit apps use Vite to build their frontends,
...
def make_resolution_plan(app: App, bench: "Bench"):
	...
	for app_name in app._get_dependencies():
		...
		is_valid_creqit_branch(dep_app.url, dep_app.branch)
...
def add_to_excluded_apps_txt(app, bench_path="."):
	if app == "creqit":
		raise ValueError("Creqit app cannot be excluded from update")
...
def get_app(
	...
	"""bench get-app clones a Creqit App from remote (GitHub or any other git server),
	...
	creqit_path, creqit_branch = None, None
	...
	if resolve_deps:
		...
		if "creqit" in resolution:
			# Todo: Make creqit a terminal dependency for all creqit apps.
			creqit_path, creqit_branch = resolution["creqit"].url, resolution["creqit"].tag
	...
	if not is_bench_directory(bench_path):
		...
		init(
			path=bench_path,
			creqit_path=creqit_path,
			creqit_branch=creqit_branch or branch,
		)
		os.chdir(bench_path)
		bench_setup = True
...
def new_app(app, no_git=None, bench_path="."):
	if bench.CREQIT_VERSION in (0, None):
		click.secho(
			f"{os.path.realpath(bench_path)} is not a valid bench directory.",
			fg="red",
		)
	...
	if no_git:
		if bench.CREQIT_VERSION < 14:
			click.secho("Creqit v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_creqit_cmd(*args, bench_path=bench_path)
...
def use_rq(bench_path):
	...
	celery_app = os.path.join(bench_path, "apps", "creqit", "creqit", "celery_app.py")
