...
def change_dir():
	if os.path.exists("config.json") or "init" in sys.argv:
		return
	dir_path_file = "/etc/creqit_bench_dir"
...
def change_uid():
	if is_root() and not cmd_requires_root():
		creqit_user = bench_config.get("creqit_user")
		if creqit_user:
			drop_privileges(uid_name=creqit_user, gid_name=creqit_user)
			os.environ["HOME"] = pwd.getpwnam(creqit_user).pw_dir
		else:
...
def app_cmd(bench_path="."):
	...
	os.execv(f, [f] + ["-m", "creqit.utils.bench_helper"] + sys.argv[1:])


def creqit_cmd(bench_path="."):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
	os.execv(f, [f] + ["-m", "creqit.utils.bench_helper", "creqit"] + sys.argv[1:])
...
def get_creqit_commands():
	...
	return set(get_env_creqit_commands())


def get_creqit_help(bench_path="."):
	python = get_env_cmd("python", bench_path=bench_path)
	sites_path = os.path.join(bench_path, "sites")
	try:
		out = get_cmd_output(
			f"{python} -m creqit.utils.bench_helper get-creqit-help", cwd=sites_path
		)
