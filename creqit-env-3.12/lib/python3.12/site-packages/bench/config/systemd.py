...
def generate_systemd_config(
	...
	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-creqit-default-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-creqit-short-worker@" + str(i + 1) + ".service"
		)

	for i in range(number_of_workers):
		background_workers.append(
			get_bench_name(bench_path) + "-creqit-long-worker@" + str(i + 1) + ".service"
		)
...
def setup_main_config(bench_info, bench_path):
	# Main config
	bench_template = bench.config.env().get_template("systemd/creqit-bench.target")
...
def setup_workers_config(bench_info, bench_path):
	...
	bench_default_worker_config_path = os.path.join(
		...
		"config",
		"systemd",
		bench_info.get("bench_name") + "-creqit-default-worker@.service",
	)
	bench_short_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-creqit-short-worker@.service",
	)
	bench_long_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-creqit-long-worker@.service",
	)
	bench_schedule_worker_config_path = os.path.join(
		bench_path,
		"config",
		"systemd",
		bench_info.get("bench_name") + "-creqit-schedule.service",
	)
...
def setup_web_config(bench_info, bench_path):
	...
	bench_web_service_config_path = os.path.join(
		bench_path, "config", "systemd", bench_info.get("bench_name") + "-creqit-web.service"
	)
...
def setup_redis_config(bench_info, bench_path):
	...
	bench_redis_target_template = bench.config.env().get_template(
		"systemd/creqit-bench-redis.target"
	)
	bench_redis_cache_template = bench.config.env().get_template(
		"systemd/creqit-bench-redis-cache.service"
	)
	bench_redis_queue_template = bench.config.env().get_template(
		"systemd/creqit-bench-redis-queue.service"
	)
...
def get_unit_files(bench_path):
	...
	unit_files = [
		...
		[bench_name + "-creqit-default-worker@", ".service"],
		[bench_name + "-creqit-short-worker@", ".service"],
		[bench_name + "-creqit-long-worker@", ".service"],
		[bench_name + "-creqit-schedule", ".service"],
		[bench_name + "-creqit-web", ".service"],
