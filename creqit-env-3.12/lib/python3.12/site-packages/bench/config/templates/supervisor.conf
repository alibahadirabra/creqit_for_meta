...
[program:{{ bench_name }}-creqit-web]
command={{ bench_dir }}/env/bin/gunicorn -b 127.0.0.1:{{ webserver_port }} -w {{ gunicorn_workers }} --max-requests {{ gunicorn_max_requests }} --max-requests-jitter {{ gunicorn_max_requests_jitter }} -t {{ http_timeout }} --graceful-timeout 30 creqit.app:application --preload
priority=4
autostart=true
autorestart=true
...
[program:{{ bench_name }}-creqit-schedule]
...
[program:{{ bench_name }}-creqit-default-worker]
...
[program:{{ bench_name }}-creqit-short-worker]
...
[program:{{ bench_name }}-creqit-long-worker]
...
[program:{{ bench_name }}-creqit-{{ worker_name }}-worker]
...
command={{ node }} {{ bench_dir }}/apps/creqit/socketio.js
priority=4
autostart=true
autorestart=true
...
programs={{ bench_name }}-creqit-web {%- if node -%} ,{{ bench_name }}-node-socketio {%- endif%}


{% if multi_queue_consumption %}

[group:{{ bench_name }}-workers]
programs={{ bench_name }}-creqit-schedule,{{ bench_name }}-creqit-short-worker,{{ bench_name }}-creqit-long-worker{%- for worker_name in workers -%},{{ bench_name }}-creqit-{{ worker_name }}-worker{%- endfor %}

{% else %}

[group:{{ bench_name }}-workers]
programs={{ bench_name }}-creqit-schedule,{{ bench_name }}-creqit-default-worker,{{ bench_name }}-creqit-short-worker,{{ bench_name }}-creqit-long-worker{%- for worker_name in workers -%},{{ bench_name }}-creqit-{{ worker_name }}-worker{%- endfor %}
