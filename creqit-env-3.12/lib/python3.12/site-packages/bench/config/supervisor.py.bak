...
def can_enable_multi_queue_consumption(bench_path: str) -> bool:
	try:
		...
		creqit_version = Version(get_current_version("creqit", bench_path=bench_path))

		return creqit_version > supported_version
	except Exception:
		return False
...
def check_supervisord_config(user=None):
	...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@creqit.io
	import configparser
