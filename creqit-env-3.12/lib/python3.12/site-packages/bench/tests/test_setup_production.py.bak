...
class TestSetupProduction(TestBenchBase):
	...
	def assert_supervisor_config(self, bench_name, use_rq=True):
		...
		with open(conf_src) as f:
			...
			tests = [
				f"program:{bench_name}-creqit-web",
			...
			if use_rq:
				tests.extend(
					[
						f"program:{bench_name}-creqit-schedule",
						f"program:{bench_name}-creqit-default-worker",
						f"program:{bench_name}-creqit-short-worker",
						f"program:{bench_name}-creqit-long-worker",
					]
				)
				...
				tests.extend(
					[
						f"program:{bench_name}-creqit-workerbeat",
						f"program:{bench_name}-creqit-worker",
						f"program:{bench_name}-creqit-longjob-worker",
						f"program:{bench_name}-creqit-async-worker",
					]
				)
	...
	def assert_supervisor_process(self, bench_name, use_rq=True, disable_production=False):
		...
		tests = [
			r"{bench_name}-web:{bench_name}-creqit-web[\s]+RUNNING",
		...
		if use_rq:
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-creqit-schedule[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-default-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-short-worker-0[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-long-worker-0[\s]+RUNNING",
				]
			)
			...
			tests.extend(
				[
					r"{bench_name}-workers:{bench_name}-creqit-workerbeat[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-longjob-worker[\s]+RUNNING",
					r"{bench_name}-workers:{bench_name}-creqit-async-worker[\s]+RUNNING",
				]
			)
