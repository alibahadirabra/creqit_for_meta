...
CREQIT_BRANCH = "version-13-hotfix"
if PYTHON_VER.major == 3:
	if PYTHON_VER.minor >= 10:
		CREQIT_BRANCH = "develop"
...
class TestBenchBase(unittest.TestCase):
	...
	def assert_folders(self, bench_name):
		...
		self.assert_exists(bench_name, "apps", "creqit")
	...
	def init_bench(self, bench_name, **kwargs):
		...
		creqit_tmp_path = "/tmp/creqit"

		if not os.path.exists(creqit_tmp_path):
			exec_cmd(
				f"git clone https://github.com/creqit/creqit -b {CREQIT_BRANCH} --depth 1 --origin upstream {creqit_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.benches_path, bench_name)):
			...
			exec_cmd(
				"git remote set-url upstream https://github.com/creqit/creqit",
				cwd=os.path.join(self.benches_path, bench_name, "apps", "creqit"),
			)
